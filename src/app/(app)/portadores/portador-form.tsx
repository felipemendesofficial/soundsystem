...
import type { PortadorFormState } from "./actions";

type Action = (prevState: PortadorFormState, formData: FormData) => Promise<PortadorFormState>;

const labelClass = "text-[15px] font-semibold";
const inputClass = "h-11 px-3.5 text-base bg-card";

export function PortadorForm({
  action,
  defaultValues,
}: {
  action: Action;
  ...
  return (
    <form action={formAction} className="max-w-md space-y-6">
      ...
      <div className="flex gap-3">
        ...
        <Button type="button" variant="outline" render={<Link href="/portadores" />} className="h-11 px-7 text-base">
          Cancelar
        </Button>
      </div>
    </form>
