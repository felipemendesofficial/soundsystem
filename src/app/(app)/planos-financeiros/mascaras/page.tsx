...
export default async function MascarasPlanoFinanceiroPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Máscaras de Plano Financeiro</h1>
        <Button render={<Link href="/planos-financeiros/mascaras/novo" />}>Nova Máscara</Button>
      </div>
      ...
      {mascaras.length === 0 ? (
        ...
        <ul className="space-y-2">
          {mascaras.map((m) => (
            <li key={m.id}>
              <Link
                href={`/planos-financeiros/mascaras/${m.id}`}
              ...
              >
                ...
                <p className="text-[13px] text-muted-foreground">
                  {m.segmentos.map((s) => s.qtdDigitos).join(".")} · {m._count.planos} conta(s)
                </p>
              </Link>
            </li>
          ))}
