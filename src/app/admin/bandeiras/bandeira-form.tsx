...
import type { BandeiraFormState } from "./actions";

type Action = (prevState: BandeiraFormState, formData: FormData) => Promise<BandeiraFormState>;

const labelClass = "text-[15px] font-semibold";
const inputClass = "h-11 px-3.5 text-base bg-card";

export function BandeiraForm({
  action,
  defaultValues,
}: {
  action: Action;
  ...
  return (
    <form action={formAction} className="max-w-md space-y-6">
      ...
      <div className="flex gap-3">
        ...
        <Button type="button" variant="outline" render={<Link href="/admin/bandeiras" />} className="h-11 px-7 text-base">
          Cancelar
        </Button>
      </div>
    </form>
